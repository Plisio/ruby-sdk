  ...
  class InvoicePaysysDto
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `PlisioAPIClient::InvoicePaysysDto` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `PlisioAPIClient::InvoicePaysysDto`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          contract_standard == o.contract_standard && super(o)
    end
    ...
    def hash
      [amount, name, cid, currency, icon, rate_usd, price_usd, precision, output_precision, fiat, fiat_rate, min_sum_in, invoice_commission_percentage, hidden, maintenance, contract_of, contract_standard].hash
    end
